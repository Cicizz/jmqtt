...
class RetainFlagTest extends AbstractMqtt5Specification{
    ...
    def "check the retained message is not removed after the message is consumed by another subscriber"(){
        given:
            ...
            def newSubscriber = basicClientFactory.createMqtt5Client().toAsync()
        and:
        ...
        when:
            ...
            subscribeMessage(subscriber, "UP/" + topic, qos, { Mqtt5Publish pub ->
                subscriberCheckPoint.addAndGet(1)
                printReceivedMessage(pub)
            })

            //subscribe with new subscribe
            subscribeMessage(newSubscriber, "UP/" + topic, qos, { Mqtt5Publish pub ->
