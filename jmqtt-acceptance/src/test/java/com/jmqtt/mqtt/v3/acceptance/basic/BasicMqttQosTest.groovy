...
class BasicMqttQosTest extends AbstractMqtt3Specification{

    static final Logger logger = LoggerFactory.getLogger(BasicMqttQosTest.class)
    ...
    def "subscribe first, receive the message, publisher qos is #qos, subscriber qos is #qos"(){
        given:
            def subscriber = basicClientFactory.createMqtt3Client()
            def publisher = basicClientFactory.createMqtt3Client()
            def checkPoint = new AtomicInteger(0)
        when:
            simpleConnect(publisher)
            simpleConnect(subscriber)

            subscribeMessage(subscriber, topic, qos, { Mqtt3Publish pub ->
